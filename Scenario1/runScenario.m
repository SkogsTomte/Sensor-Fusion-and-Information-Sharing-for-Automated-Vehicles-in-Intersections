...
%             lastObjects(smallestIndex).uppdate(coord,vel,frameRate,allData(t-1).ActorPoses(3),3);
%             lastObjects(smallestIndex).detected = 0;%These are changed to 1 by the uppdate method but should
%             lastObjects(smallestIndex).found = 0;   %in this case remain 0 until the next section of the code
        end
    end
    ...
    for i=1:length(lastObjects)
        if ((lastObjects(i).detected == 0) && (lastObjects(i).lost == 0))
            ...
            %posGuess = lastObjects(i).pos;
            %velGuess = lastObjects(i).vel;
            if t~=2
